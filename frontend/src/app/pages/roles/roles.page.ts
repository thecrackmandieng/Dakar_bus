...
export class RolesPage implements OnInit {
  ...
  edit(item: Roles): void {
    this.editingId = item.idRole as string | number;
  ...
  async save(): Promise<void> {
    ...
    try {
      if (this.editingId === null) {
        await this.service.create(this.form as Partial<Roles>);
        this.successMessage = 'Element cree avec succes.';
      } else {
        await this.service.update(this.editingId, this.form as Partial<Roles>);
  ...
  async confirmRemove(): Promise<void> {
    ...
    this.refresh();
    try {
      await this.service.remove(this.pendingDelete.idRole as string | number);
      this.closeConfirm();
      await this.load();
