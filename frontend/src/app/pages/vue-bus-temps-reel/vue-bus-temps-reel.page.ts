...
import { VueBusTempsReelService } from './vue-bus-temps-reel.service';

@Component({
  selector: 'app-vue-bus-temps-reel',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './vue-bus-temps-reel.page.html',
  styleUrl: './vue-bus-temps-reel.page.scss'
})
export class VueBusTempsReelPage implements OnInit {
  items: VueBusTempsReel[] = [];
  ...
  successOpen = false;
  successMessage = '';
  pendingDelete: VueBusTempsReel | null = null;

  constructor(
    private service: VueBusTempsReelService,
  ...
  edit(item: VueBusTempsReel): void {
  ...
  async save(): Promise<void> {
    ...
    try {
      if (this.editingId === null) {
        await this.service.create(this.form as Partial<VueBusTempsReel>);
        this.successMessage = 'Element cree avec succes.';
      } else {
        await this.service.update(this.editingId, this.form as Partial<VueBusTempsReel>);
  ...
  askRemove(item: VueBusTempsReel): void {
