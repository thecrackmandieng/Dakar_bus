...
export class LignesPage implements OnInit {
  ...
  successOpen = false;
  successMessage = '';
  pendingDelete: Lignes | null = null;

  constructor(
    private service: LignesService,
  ...
  edit(item: Lignes): void {
    this.editingId = item.idLigne as string | number;
  ...
  async save(): Promise<void> {
    ...
    try {
      if (this.editingId === null) {
        await this.service.create(this.form as Partial<Lignes>);
        this.successMessage = 'Element cree avec succes.';
      } else {
        await this.service.update(this.editingId, this.form as Partial<Lignes>);
  ...
  async confirmRemove(): Promise<void> {
    ...
    this.refresh();
    try {
      await this.service.remove(this.pendingDelete.idLigne as string | number);
      this.closeConfirm();
      await this.load();
