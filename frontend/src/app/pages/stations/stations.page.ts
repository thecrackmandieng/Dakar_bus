...
export class StationsPage implements OnInit {
  ...
  successOpen = false;
  successMessage = '';
  pendingDelete: Stations | null = null;
  ...
  edit(item: Stations): void {
    this.editingId = item.idStation as string | number;
  ...
  async save(): Promise<void> {
    ...
    try {
      if (this.editingId === null) {
        await this.service.create(this.form as Partial<Stations>);
        this.successMessage = 'Element cree avec succes.';
      } else {
        await this.service.update(this.editingId, this.form as Partial<Stations>);
  ...
  async confirmRemove(): Promise<void> {
    ...
    this.refresh();
    try {
      await this.service.remove(this.pendingDelete.idStation as string | number);
      this.closeConfirm();
      await this.load();
