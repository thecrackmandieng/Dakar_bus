...
import { Trajets } from './trajets.model';
...
export class TrajetsPage implements OnInit {
  ...
  successOpen = false;
  successMessage = '';
  pendingDelete: Trajets | null = null;

  constructor(
    private service: TrajetsService,
  ...
  edit(item: Trajets): void {
    this.editingId = item.idTrajet as string | number;
  ...
  async save(): Promise<void> {
    ...
    try {
      if (this.editingId === null) {
        await this.service.create(this.form as Partial<Trajets>);
        this.successMessage = 'Element cree avec succes.';
      } else {
        await this.service.update(this.editingId, this.form as Partial<Trajets>);
  ...
  async confirmRemove(): Promise<void> {
    ...
    this.refresh();
    try {
      await this.service.remove(this.pendingDelete.idTrajet as string | number);
      this.closeConfirm();
      await this.load();
